...
def tambah_transaksi(customer_id):
    ...
    try:
        ...
        # Check if the column 'waktu' exists in 'detail_layanan'
        cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='detail_layanan' AND column_name='waktu'")
        column_exists = cur.fetchone()
        if not column_exists:
            # Add the 'waktu' column if it does not exist
            cur.execute("ALTER TABLE detail_layanan ADD COLUMN waktu INT")
            conn.commit()


            # Set default values for 'waktu' based on the example you provided
            default_waktu = {1: 5, 2: 3, 3: 5, 4: 3}
            for id_detail, waktu in default_waktu.items():
                cur.execute("UPDATE detail_layanan SET waktu = %s WHERE id_detail = %s", (waktu, id_detail))
            conn.commit()
        ...
        SELECT
            ...
            ll.nama AS layanan_laundry, dl.waktu as Waktu
        FROM
            detail_layanan dl
        JOIN
        ...
        tgl_pickup_str = input("Tanggal Pick-Up (YYYY-MM-DD): ")
        tgl_pickup = datetime.strptime(tgl_pickup_str, "%Y-%m-%d")


        # Get the duration (waktu) for the selected package
        cur.execute("SELECT waktu FROM detail_layanan WHERE id_detail = %s", (jenis_paket,))
        ...
        # Insert new transaction and calculate delivery date
        query = """
        INSERT INTO transaksi (
            pelanggan_id_pelanggan, jenis_parfum_id_parfum, detail_layanan_id_detail,
            ttl_diterima, ttl_selesai, mtd_bayar_id_pembayaran, subtotal, ttl_brt, pegawai_id_pegawai, stat_bayar
        ) VALUES (%s, %s, %s, %s, %s::DATE + interval '%s day', %s, %s, %s, %s, %s) RETURNING id_transaksi
        """
        subtotal = 0  # Replace with appropriate subtotal calculation
        ttl_brt = 0.0  # Replace with appropriate total weight calculation
        pegawai_id = 2  # Replace with actual Pegawai ID if needed
        stat_bayar = 'Belum Lunas'
        cur.execute(query, (customer_id, jenis_parfum, jenis_paket, tgl_pickup_str, tgl_pickup_str, waktu, metode_pembayaran, subtotal, ttl_brt, pegawai_id, stat_bayar))
        ...
        if transaksi_baru:
            headers = ["ID Transaksi", "Tanggal Diterima", "Tanggal Selesai", "Jenis Paket", "Layanan Laundry", "Jenis Parfum", "Metode Pembayaran", "Subtotal", "Total Berat", "Pegawai", "Status Pembayaran"]
...
def bayar_transaksi(customer_id):
    ...
    try:
        ...
        JOIN
            pelanggan pl ON tr.pelanggan_id_pelanggan = pl.id_pelanggan
        WHERE
            pl.id_pelanggan = %s AND tr.stat_bayar = 'Belum Lunas' and tr.subtotal <> '0'
        """
        cur.execute(query_histori, (customer_id,))
        transactions = cur.fetchall()
        ...
        if not transactions:
            print("Tidak ada transaksi yang ditemukan untuk pelanggan ini atau total berat belum diinputkan oleh admin.")
            return
        ...
        id_transaksi, subtotal, total_berat, metode_pembayaran = transaksi
        print(f"Subtotal: {subtotal}, Total Berat: {total_berat}, Metode Pembayaran: {metode_pembayaran}")
       
        # Process payment based on the method stored in the transaction
        if metode_pembayaran == 'cash':  # Assuming 'Cash' is the method for ID 1
        ...
        elif metode_pembayaran == 'transfer':  # Assuming 'Transfer' is the method for ID 2
...
def cancel_transaction(customer_id):
    try:
        ...
        JOIN
            pelanggan pl ON tr.pelanggan_id_pelanggan = pl.id_pelanggan
        WHERE
            pl.id_pelanggan = %s and tr.stat_bayar = 'Belum Lunas'
        """
        cur.execute(query_histori, (customer_id,))
        transactions = cur.fetchall()
